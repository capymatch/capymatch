        ...
        if existing:
            if update_data:
                ...
                await db.university_knowledge_base.update_one(
                    {"university_name": name}, {"$set": update_data}
                )
                updated += 1
        else:
...
async def get_university(university_name: str):
    uni = await db.university_knowledge_base.find_one(
        {"university_name": university_name}, {"_id": 0}
    )
    if not uni:
        raise HTTPException(status_code=404, detail="University not found")

    # Get how many athletes have this school on their board
...
async def update_university(university_name: str, request: Request):
    ...
    if update_data:
        update_data["updated_at"] = datetime.now(timezone.utc).isoformat()
        await db.university_knowledge_base.update_one(
            {"university_name": university_name}, {"$set": update_data}
        )
