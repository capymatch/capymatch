...
import { Users, UserPlus, Crown, User, Trash2, Loader2, Mail, X, ArrowUpRight } from "lucide-react";
...
export default function TeamSection() {
  ...
  const handleLeave = async () => {
    if (!window.confirm("Leave this team? You'll go back to your own account.")) return;
    try {
  ...
  const atLimit = maxMembers !== -1 && currentCount >= maxMembers && team.pending_invitations.length === 0;
          ...
          {team.pending_invitations.map((inv) => (
            <div key={inv.invite_id} className="flex items-center gap-3 px-4 py-3 rounded-xl border border-dashed" style={{ borderColor: "var(--t-border)" }} data-testid={`pending-invite-${inv.invite_id}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px]" style={{ color: "var(--t-text-muted)" }}>Invitation pending</p>
              </div>
              {isOwner && (
                <button
        ...
        <>
          {canInvite ? (
            ...
            <div className="flex items-center justify-between px-4 py-3 rounded-xl border border-dashed" style={{ borderColor: "rgba(244,63,94,0.3)", backgroundColor: "rgba(244,63,94,0.05)" }}>
              ...
              <a href="/settings" onClick={(e) => { e.preventDefault(); }} className="flex items-center gap-1 text-xs font-semibold text-pink-400 hover:text-pink-300 transition-colors">
