...
import { MapPin, Star, Loader2, Mail, ExternalLink, Eye } from "lucide-react";
...
function getEmailEngagement(event, engagement) {
  ...
  // 1. Try exact subject match (for Gmail emails)
  ...
  // 2. For logged/manual sent emails, match by content keywords
  if (matched.length === 0) {
  ...
  // 3. Include orphaned link_clicks (no subject) if we already matched some opens
  if (matched.length > 0) {
    const orphanClicks = engagement.timeline.filter(e =>
      e.event_type === "link_click" && !(e.email_subject || "").trim() && !matched.includes(e)
    );
    if (orphanClicks.length > 0) matched = [...matched, ...orphanClicks];
  }
  ...
  return (
    <div className="absolute top-2.5 right-3 group/badge z-10" data-testid="engagement-badge">
      ...
      <div className="absolute bottom-full right-0 mb-2 w-56 rounded-xl p-2.5 opacity-0 pointer-events-none group-hover/badge:opacity-100 group-hover/badge:pointer-events-auto transition-opacity shadow-xl"
        ...
        <div className="absolute top-full right-4 w-0 h-0 border-x-[6px] border-x-transparent border-t-[6px]" style={{ borderTopColor: "#1a1a2e" }} />
      </div>
    </div>
  );
}
  ...
  return (
    <div className={`flex ${isRight ? "justify-end" : "justify-start"} my-1`} data-testid={`conv-bubble-${isRight ? "right" : "left"}`}>
      <div className={`max-w-[80%] sm:max-w-[70%] rounded-2xl px-4 py-3 border relative ${
        ...
        <p className={`text-[10px] font-bold uppercase tracking-wider mb-1 ${isRight ? "text-teal-700" : "text-slate-500"}`} style={isRight && matchedEngagement ? { paddingRight: "48px" } : {}}>
