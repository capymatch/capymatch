...
export default function Analytics() {
  ...
  const statusColors = {
    ...
    "Active Conversation": "#1a8a80", "Offer Received": "#10b981", "Not a Fit / Closed": "#ef4444",
  };
            ...
            {statusEntries.length > 0 ? (
              <div className="space-y-3">
                {statusEntries.map(([status, count]) => (
                  <HBar key={status} label={status} value={count} max={maxStatus} color={statusColors[status] || "#1a8a80"} />
                ))}
              </div>
            ) : (
