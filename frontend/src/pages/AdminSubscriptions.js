...
const PLAN_PRICES = { basic: 0, pro: 19, premium: 39 };
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-xl font-bold" style={{ color: "var(--t-text)" }}>{value}</p>
          <p className="text-[11px]" style={{ color: "var(--t-text-muted)" }}>{label}</p>
        </div>
      </div>
      {sub && <p className="text-[10px] mt-2 pl-[52px]" style={{ color: "var(--t-text-muted)" }}>{sub}</p>}
    </div>
  );
}
...
function InlinePlanChanger({ userId, currentPlan, onChanged }) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await api.put(`/admin/subscriptions/${userId}`, { plan, reason: "Bulk admin change" });
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [subRes, logRes] = await Promise.all([
        ...
        api.get("/admin/subscription-logs", { params: { limit: 10 } }),
      ]);
      setData(subRes.data);
    ...
    <div className="space-y-5" data-testid="admin-subscriptions-page">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <StatCard
          icon={TrendingUp}
          label="Free Users"
          value={stats.plan_counts.basic}
          sub="Upgrade potential"
          color="bg-zinc-600/15 text-zinc-400"
        />
      </div>

      {/* Plan Distribution Mini Bars */}
              ...
              <div key={p} className="flex-1">
                <div className="flex items-center justify-between mb-1.5">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-xs font-medium" style={{ color: "var(--t-text)" }}>{p.charAt(0).toUpperCase() + p.slice(1)}</span>
                  </div>
                  <span className="text-xs font-bold" style={{ color: "var(--t-text)" }}>{count} ({pct}%)</span>
                </div>
        ...
        <div className="xl:col-span-2 space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Select value={planFilter} onValueChange={(v) => { setPlanFilter(v); setPage(1); }}>
              <SelectTrigger className="w-32 text-xs" style={{ backgroundColor: "var(--t-surface)", borderColor: "var(--t-border)", color: "var(--t-text-secondary)" }} data-testid="sub-plan-filter">
          ...
          <div className="rounded-xl border overflow-hidden" style={{ backgroundColor: "var(--t-surface)", borderColor: "var(--t-border)" }}>
              ...
              <table className="w-full text-sm" data-testid="sub-users-table">
                <thead>
                  <tr className="border-b" style={{ borderColor: "var(--t-border)" }}>
                    ...
                    <th className="text-left px-4 py-3 text-xs font-medium hidden md:table-cell" style={{ color: "var(--t-text-muted)" }}>Schools</th>
                    <th className="text-left px-4 py-3 text-xs font-medium hidden lg:table-cell" style={{ color: "var(--t-text-muted)" }}>AI Used</th>
                    <th className="text-left px-4 py-3 text-xs font-medium hidden lg:table-cell" style={{ color: "var(--t-text-muted)" }}>Joined</th>
                  </tr>
                </thead>
                <tbody>
                  {loading ? (
                    <tr><td colSpan={5} className="text-center py-12"><div className="w-6 h-6 border-2 border-slate-200 border-t-slate-600 rounded-full animate-spin mx-auto" /></td></tr>
                  ) : (data?.users || []).length === 0 ? (
                    <tr><td colSpan={5} className="text-center py-12 text-xs" style={{ color: "var(--t-text-muted)" }}>No users found</td></tr>
                  ) : (
                    (data?.users || []).map((u) => {
                      const badge = PLAN_BADGE[u.plan] || PLAN_BADGE.basic;
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 hidden md:table-cell">
                            <div className="flex items-center gap-2">
                              <span className={`text-xs font-medium ${overLimit ? "text-red-400" : ""}`} style={overLimit ? {} : { color: "var(--t-text)" }}>
                                {u.school_count}/{u.school_limit === -1 ? "\u221e" : u.school_limit}
                              </span>
                              <div className="w-16 h-1.5 rounded-full overflow-hidden" style={{ backgroundColor: "var(--t-border)" }}>
                                <div
                                  className="h-full rounded-full"
                                  style={{
                                    width: `${u.school_limit === -1 ? 15 : Math.min(100, schoolPct)}%`,
                                    backgroundColor: overLimit ? "#ef4444" : schoolPct > 80 ? "#f59e0b" : "#1a8a80",
                                  }}
                                />
                              </div>
                            </div>
                          ...
                          <td className="px-4 py-3 hidden lg:table-cell">
                            <span className="text-xs" style={{ color: "var(--t-text-muted)" }}>{formatDate(u.created_at)}</span>
                          </td>
                        </tr>
                      );
                    })
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between px-4 py-3 border-t" style={{ borderColor: "var(--t-border)" }}>
                <span className="text-xs" style={{ color: "var(--t-text-muted)" }}>{data?.total || 0} users</span>
          ...
          <div className="rounded-xl border overflow-hidden" style={{ backgroundColor: "var(--t-surface)", borderColor: "var(--t-border)" }} data-testid="sub-audit-log">
            {logs.length === 0 ? (
              <div className="text-center py-10 px-4">
                <Clock className="w-8 h-8 mx-auto mb-2 opacity-20" style={{ color: "var(--t-text-muted)" }} />
                <p className="text-xs" style={{ color: "var(--t-text-muted)" }}>No subscription changes yet</p>
                <p className="text-[10px] mt-1" style={{ color: "var(--t-text-faint)" }}>Changes will appear here when plans are modified</p>
              </div>
            ) : (
                    ...
                    <div key={log.log_id} className="px-4 py-3" data-testid={`sub-log-${log.log_id}`}>
                      <div className="flex items-start gap-2.5">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[10px] mt-0.5" style={{ color: "var(--t-text-faint)" }}>
