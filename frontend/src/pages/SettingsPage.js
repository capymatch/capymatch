...
import { Moon, Sun, Monitor, Palette, Mail, CheckCircle, XCircle, Loader2, Sparkles, Shield, Download, Trash2, Eye, ExternalLink, User, Pencil, Check, X as XIcon } from "lucide-react";
...
export default function SettingsPage() {
  ...
  const hasInfoChanges = accountName !== originalName || accountEmail !== originalEmail;
  ...
  const handleSaveInfo = async (e) => {
    ...
    try {
      ...
      setOriginalEmail(accountEmail.trim());
      setIsEditing(false);
      toast.success("Account info updated");
    } catch (err) {
      toast.error(err?.response?.data?.detail || "Failed to update account info");
    } finally {
      setInfoSaving(false);
    }
  };
  ...
  return (
    <div data-testid="settings-page" className="max-w-3xl mx-auto space-y-5 lg:space-y-8">
      {/* Personal Info */}
      <div className="rounded-xl p-4 lg:p-6 border" style={{ backgroundColor: "var(--t-surface)", borderColor: "var(--t-border)" }} data-testid="settings-personal-info">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 lg:w-10 lg:h-10 rounded-lg flex items-center justify-center" style={{ background: "rgba(46,196,182,0.2)" }}>
              <User className="w-4 h-4 lg:w-5 lg:h-5" style={{ color: "#2ec4b6" }} />
            </div>
            <div>
              <h2 className="font-semibold text-base lg:text-lg" style={{ color: "var(--t-text)" }}>Personal Info</h2>
              <p className="text-xs lg:text-sm" style={{ color: "var(--t-text-muted)" }}>Your account name and email</p>
            </div>
          </div>
          {!isEditing && !infoLoading && (
            <button
              ...
              data-testid="settings-edit-info-btn"
            >
        ...
        {infoLoading ? (
          ...
          <form onSubmit={handleSaveInfo} className="space-y-4">
            ...
            <div>
              ...
              />
              {isGoogleUser && (
                <p className="text-[11px] mt-1" style={{ color: "var(--t-text-muted)" }}>Email is managed by Google and cannot be changed here.</p>
              )}
            </div>
            <div className="flex items-center gap-2 pt-1">
              <button type="submit" disabled={infoSaving || !hasInfoChanges}
                data-testid="settings-save-info-btn"
              ...
              <button type="button" onClick={() => { setAccountName(originalName); setAccountEmail(originalEmail); setIsEditing(false); }}
                ...
                data-testid="settings-cancel-edit-btn"
              >
                Cancel
              </button>
            </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between p-3 rounded-xl" style={{ backgroundColor: "var(--t-surface-alt)" }}>
              <div>
                ...
                <p className="text-sm font-medium mt-0.5" style={{ color: "var(--t-text)" }} data-testid="settings-name-display">{originalName || "—"}</p>
              </div>
            </div>
            <div className="flex items-center justify-between p-3 rounded-xl" style={{ backgroundColor: "var(--t-surface-alt)" }}>
              <div>
                <p className="text-[11px] font-medium uppercase tracking-wider" style={{ color: "var(--t-text-muted)" }}>Email</p>
                <p className="text-sm font-medium mt-0.5" style={{ color: "var(--t-text)" }} data-testid="settings-email-display">{originalEmail || "—"}</p>
              </div>
              {isGoogleUser && (
