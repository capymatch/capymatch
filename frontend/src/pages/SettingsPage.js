    ...
    <div data-testid="settings-page" className="max-w-3xl mx-auto space-y-5 lg:space-y-8">
      ...
      <div className="rounded-xl p-4 lg:p-6 border" style={{ backgroundColor: "var(--t-surface)", borderColor: "var(--t-border)" }}>
        <div className="flex items-center gap-3 mb-4 lg:mb-6">
          <div className="w-9 h-9 lg:w-10 lg:h-10 rounded-lg bg-pink-600/20 flex items-center justify-center">
            <Palette className="w-4 h-4 lg:w-5 lg:h-5 text-pink-600" />
          </div>
          <div>
        ...
        <div className="grid grid-cols-3 gap-2 lg:gap-4">
          {themeOptions.map((option) => (
            <button
              ...
              className={`p-3 lg:p-4 rounded-xl border-2 transition-all flex flex-col items-center text-center ${theme === option.value ? "border-pink-600 bg-pink-600/10" : ""}`}
            ...
            >
              <div className={`w-9 h-9 lg:w-10 lg:h-10 rounded-lg flex items-center justify-center mb-2 ${theme === option.value ? "bg-pink-600/30" : ""}`} style={{ backgroundColor: theme === option.value ? undefined : "var(--t-surface)" }}>
                <option.icon className={`w-4 h-4 lg:w-5 lg:h-5 ${theme === option.value ? "text-pink-600" : ""}`} style={{ color: theme === option.value ? undefined : "var(--t-text-muted)" }} />
              </div>
      ...
      <div className="rounded-xl p-4 lg:p-6 border" style={{ backgroundColor: "var(--t-surface)", borderColor: "var(--t-border)" }}>
        ...
        {gmailLoading ? (
          ...
          <div className="flex items-center gap-3 p-3 rounded-xl" style={{ backgroundColor: "var(--t-surface-alt)" }}>
            ...
            <button data-testid="connect-gmail-settings-btn" onClick={handleConnectGmail} className="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg font-medium text-white bg-pink-700 hover:bg-pink-800 transition-colors">
