...
/* ── Board Groups Config ── */
const BOARD_GROUPS = [
  { key: "action_required", label: "Action Required", icon: AlertTriangle, dot: "bg-red-500", countBg: "bg-red-500/10", countText: "text-red-500", accentBar: "bg-red-500", description: "Overdue, needs response, or stale" },
  { key: "upcoming", label: "Upcoming", icon: Clock, dot: "bg-amber-500", countBg: "bg-amber-500/10", countText: "text-amber-600", accentBar: "bg-amber-500", description: "Follow-up due within 14 days" },
  { key: "in_progress", label: "In Progress", icon: Activity, dot: "bg-emerald-500", countBg: "bg-emerald-500/10", countText: "text-emerald-600", accentBar: "bg-emerald-500", description: "Recently contacted or active conversation" },
  { key: "closed", label: "Closed", icon: Archive, dot: "bg-slate-400", countBg: "bg-slate-400/10", countText: "text-slate-500", accentBar: "bg-slate-400", description: "Not a fit, committed, or archived" },
];
  ...
  return (
    <div className="flex items-center gap-1 p-1 rounded-[10px] border overflow-x-auto" style={{ backgroundColor: "var(--t-surface)", borderColor: "var(--t-border)" }} data-testid="group-funnel">
      <div
        ...
        className={`flex items-center gap-2 px-3 py-2 rounded-lg justify-center cursor-pointer transition-all flex-shrink-0 ${!activeFilter ? "bg-pink-600 text-white shadow-sm" : "hover:bg-[var(--t-surface-alt)]"}`}
        data-testid="funnel-all"
      >
        <span className={`text-xs font-medium ${!activeFilter ? "text-white" : ""}`} style={activeFilter ? { color: "var(--t-text-secondary)" } : {}}>All</span>
      ...
      {BOARD_GROUPS.map((group) => {
        ...
        return (
          <div
            ...
            className={`flex items-center gap-2 px-3 py-2 rounded-lg flex-1 min-w-fit justify-center cursor-pointer transition-all flex-shrink-0 ${isActive ? "bg-pink-600 text-white shadow-sm" : "hover:bg-[var(--t-surface-alt)]"}`}
            data-testid={`funnel-${group.key}`}
          >
            <span className={`text-xs font-medium hidden sm:inline ${isActive ? "text-white" : group.countText}`}>{group.label}</span>
            <span className={`text-sm font-bold ${isActive ? "text-white" : ""}`} style={isActive ? {} : { color: "var(--t-text)" }}>{count}</span>
          </div>
        );
      })}
    </div>
...
function ProgramRow({ p, navigate, matchScore, accentColor }) {
  ...
  const scoreColor = matchScore?.match_score >= 80 ? "text-emerald-700 bg-emerald-100 border-emerald-200 dark:text-emerald-400 dark:bg-emerald-500/15 dark:border-emerald-500/30"
    : matchScore?.match_score >= 60 ? "text-amber-700 bg-amber-100 border-amber-200 dark:text-amber-400 dark:bg-amber-500/15 dark:border-amber-500/30"
    : "text-slate-600 bg-slate-100 border-slate-200 dark:text-gray-400 dark:bg-gray-500/15 dark:border-gray-500/30";
      ...
      <div className="flex items-center gap-3 flex-shrink-0">
        {p.board_group === "action_required" && (() => {
          const alert = isOverdue
            ? { text: `Overdue since ${dueDateFormatted}`, color: "text-red-500" }
            : p.recruiting_status === "Not Contacted"
            ? { text: "Not contacted yet", color: "text-red-500" }
            : p.reply_status === "No Reply" && p.recruiting_status !== "Not Contacted"
            ? { text: "No reply yet", color: "text-amber-600 dark:text-amber-400" }
            : p.reply_status === "Awaiting Reply"
            ? { text: "Awaiting reply", color: "text-amber-600 dark:text-amber-400" }
            : { text: "Needs attention", color: "text-red-500" };
        ...
        {p.board_group === "upcoming" && p.next_action_due && (
          <span className="hidden sm:block text-[11px] font-medium text-amber-600 dark:text-amber-400 whitespace-nowrap">Due {dueDateFormatted}</span>
        )}
        {p.board_group === "in_progress" && (
          <span className="hidden sm:block text-[11px] font-medium text-emerald-600 dark:text-emerald-400 whitespace-nowrap">
            {p.reply_status === "In Conversation" ? "Active conversation" : "Recently contacted"}
          </span>
        )}
        <button
          onClick={() => navigate(`/journey/${p.program_id}`)}
          className="flex items-center gap-1.5 px-3.5 py-1.5 text-[11px] font-semibold text-white bg-pink-600 hover:bg-pink-700 rounded-full transition-colors shadow-sm"
