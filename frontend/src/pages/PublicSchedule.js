...
const THEME_CSS = `
  ...
  .profile-root {
    --p-bg: #e8e7ef;
    --p-hero-bg: #ffffff;
    --p-hero-glow: rgba(26,138,128,0.05);
    --p-surface: #ffffff;
    ...
    --p-text-faint: #b0b0c0;
    --p-accent: #1a8a80;
    --p-accent-glow: rgba(26,138,128,0.08);
    --p-accent-glow-strong: rgba(26,138,128,0.12);
    --p-stat-bg: #e8eaf8;
    --p-stat-border: rgba(99,102,241,0.18);
    --p-stat-highlight-bg: #fde8ed;
    --p-stat-highlight-border: rgba(26,138,128,0.25);
    --p-section-bg: #ffffff;
    ...
    --p-sticky-bg: rgba(232,231,239,0.92);
    --p-sticky-secondary-bg: #ffffff;
    --p-share-bg: rgba(0,0,0,0.05);
    --p-share-hover: rgba(0,0,0,0.10);
    --p-share-border: rgba(0,0,0,0.08);
    --p-share-icon: #1a1a2e;
    --p-divider: rgba(0,0,0,0.08);
    --p-empty-bg: #f8f7fc;
  }
`;

const EVENT_LIGHT_STYLES = {
  Camp: { bg: "rgba(26,138,128,0.08)", color: "#d63b5c" },
  ...
  Tryout: { bg: "rgba(26,138,128,0.08)", color: "#d63b5c" },
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: "var(--p-bg, #f6f6f9)" }}>
        <Loader2 className="w-8 h-8 animate-spin text-[#1a8a80]" />
      </div>
    );
  }
    ...
    <div data-testid="public-schedule" className="profile-root min-h-screen" style={{ background: "var(--p-bg)", fontFamily: "'DM Sans', -apple-system, sans-serif" }}>
        ...
        <div className="relative z-10 max-w-[860px] mx-auto px-6 sm:px-8 py-14 sm:py-16 flex flex-col sm:flex-row items-center sm:items-start gap-8 sm:gap-12">
          ...
          <div className="flex-shrink-0 relative" data-testid="hero-photo-wrapper">
            {hasPhoto ? (
              ...
              <div className="w-52 h-64 sm:w-[280px] sm:h-[340px] rounded-[20px] bg-gradient-to-br from-[#1a8a80] to-indigo-500 flex items-center justify-center shadow-lg">
                <User className="w-20 h-20 text-white/70" />
              </div>
            )}
            <div className="absolute -bottom-1.5 left-6 right-6 h-[3px] rounded-full" style={{ background: "linear-gradient(90deg, var(--p-accent), rgba(26,138,128,0.15))" }} />
          </div>
            ...
            <div className="hidden sm:flex gap-2.5 mt-8">
              ...
              {profile.video_link && (
                <a href={profile.video_link} target="_blank" rel="noopener noreferrer" data-testid="video-link"
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-[13px] font-semibold text-white bg-[#1a8a80] hover:brightness-110 transition-all hover:-translate-y-px shadow-sm">
                ...
                <div className="flex gap-2">
                  {profile.parent_email && (
                    <a href={`mailto:${profile.parent_email}`} title="Email Coach"
                      ...
                      <Mail className="w-4 h-4 group-hover:text-[#1a8a80] transition-colors" style={{ color: "var(--p-text-muted)" }} />
                    </a>
                  )}
                  {profile.parent_phone && (
                    <a href={`tel:${profile.parent_phone}`} title="Call Coach"
                      className="w-[40px] h-[40px] rounded-xl flex items-center justify-center transition-all group"
                      style={{ background: "#f2f3f8", border: "1px solid rgba(0,0,0,0.06)" }}>
                      <Phone className="w-4 h-4 group-hover:text-[#1a8a80] transition-colors" style={{ color: "var(--p-text-muted)" }} />
                    </a>
                  )}
                </div>
              </div>
            ...
            <div className="rounded-2xl p-6 sm:p-8" style={{ background: "var(--p-section-bg)", border: "1px solid var(--p-section-border)", boxShadow: "var(--p-section-shadow)" }} data-testid="video-embed-section">
              ...
              {videoPlaying ? (
                ...
                >
                  ...
                  />
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[68px] h-[68px] rounded-full bg-[rgba(26,138,128,0.9)] backdrop-blur-sm flex items-center justify-center group-hover:scale-110 transition-transform">
      ...
      >
        <div className="flex gap-2 px-4 pt-3 max-w-[500px] mx-auto">
          {profile.contact_email && (
            <a href={`mailto:${profile.contact_email}`} className="flex-1 flex items-center justify-center gap-1.5 py-3 rounded-xl text-[13px] font-semibold text-white bg-[#1a8a80] shadow-sm">
