...
function EmailAvatar({ name, email: addr }) {
  const colors = ["#6366f1", "#8b5cf6", "#1a8a80", "#f97316", "#10b981", "#3b82f6", "#ef4444", "#14b8a6"];
                  ...
                  {msg.attachments.map((att, ai) => (
                    <button
                      ...
                      onClick={async (e) => {
                        e.stopPropagation();
                        try {
                          const res = await api.get(`/gmail/attachments/${msg.id}/${att.attachment_id}`, { responseType: "blob" });
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {em.is_known_coach ? (
                          <span className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-slate-500/15 text-slate-500 flex-shrink-0">Coach</span>
                        ) : (
