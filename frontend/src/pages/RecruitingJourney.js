...
export default function RecruitingJourney() {
  ...
  return (
    <div data-testid="recruiting-journey" className="max-w-6xl mx-auto pb-24">
      ...
      {/* ─── Inline Forms ─── */}
      {activeForm === "replied" && <div className="mt-3"><MarkAsRepliedModal programId={programId} onSaved={() => { closeForm(); fetchData(); }} onCancel={closeForm} /></div>}
      {activeForm === "log" && <div className="mt-3"><LogInteractionForm programId={programId} universityName={program.university_name} onSaved={() => { closeForm(); fetchData(); }} onCancel={closeForm} /></div>}
      {activeForm === "email" && <div className="mt-3"><EmailComposer coaches={coaches} programId={programId} onSent={() => { closeForm(); fetchData(); }} onCancel={closeForm} /></div>}
      {activeForm === "coach" && <div className="mt-3"><CoachForm initial={editCoach} programId={programId} onSave={saveCoach} onCancel={closeForm} /></div>}
      {activeForm === "followup" && (
        <div className="mt-3 rounded-xl border p-4" style={{ backgroundColor: "var(--t-surface)", borderColor: "var(--t-border)" }}>
