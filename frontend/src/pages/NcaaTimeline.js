  ...
  return (
    <div className="rounded-xl border p-4 sm:p-5" style={{ backgroundColor: "var(--t-surface)", borderColor: "var(--t-border)" }} data-testid="ncaa-timeline-bar">
      ...
      <div className="flex mb-1.5 overflow-x-auto">
        {MONTHS.map((m, i) => (
          <div key={m} className={`flex-1 text-center text-[9px] sm:text-[10px] font-semibold uppercase tracking-wider min-w-[2rem] ${i === currentIdx ? "text-teal-500" : ""}`}
      ...
      <div className="relative mb-3">
        ...
        <div className="absolute top-[-6px] bottom-[-6px] w-[2px] bg-teal-600 z-10" style={{ left: `${nowPercent}%` }}>
          <span className="absolute -top-4 left-1/2 -translate-x-1/2 text-[7px] font-extrabold tracking-wider text-teal-500 bg-[var(--t-surface)] px-1.5 py-0.5 rounded border border-teal-600/30">NOW</span>
        </div>
      </div>
        ...
        {DIVISIONS.map(d => (
          <button key={d} onClick={() => setDivision(d)}
            className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-colors ${
              d === division ? "bg-teal-600/15 border-teal-600/40 text-teal-500" : "border-[var(--t-border)] hover:border-teal-600/30"
            }`}
